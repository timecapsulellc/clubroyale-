...
void main() {
  group('Nepali Marriage Rules Verification', () {
    ...
    test('WildcardHelper recognizes all Nepali wildcards', () {
      ...
      expect(
        helper.isWildcard(poplu),
        isTrue,
        reason: 'Poplu (Rank+1) should be wild',
      );
      expect(
        helper.isWildcard(jhiplu),
        isTrue,
        reason: 'Jhiplu (Rank-1) should be wild',
      );
      expect(
        helper.isWildcard(alter),
        isTrue,
        reason: 'Alter (Same Rank/Color) should be wild',
      );
      expect(
        helper.isWildcard(normal),
        isFalse,
        reason: 'Normal card should not be wild',
      );
    });

    test('DubleeMeld detects pairs from different decks', () {
      final c1 = PlayingCard(
        suit: CardSuit.hearts,
        rank: CardRank.king,
        deckIndex: 0,
      );
      final c2 = PlayingCard(
        suit: CardSuit.hearts,
        rank: CardRank.king,
        deckIndex: 1,
      );
      final c3 = PlayingCard(
        suit: CardSuit.hearts,
        rank: CardRank.king,
        deckIndex: 0,
      ); // Same deck as c1
      ...
      expect(
        invalidPair.isValid,
        isFalse,
        reason: 'Same deck cards cannot form Dublee',
      );
    });
    ...
    test('Scorer awards 5 points for Tunnel', () {
      ...
      expect(
        scores2['p1'],
        5,
        reason: 'P1 should get 5 points from P2 for Tunnel',
      );
    });
    ...
    test('Kidnap logic enforces 3 pure sequences', () {
      ...
      final melds = {
        'p1': <Meld>[
          ...
          pureSeq,
          pureSeq,
        ], // Winner visited (irrelevant for kidnap check on victim?)
