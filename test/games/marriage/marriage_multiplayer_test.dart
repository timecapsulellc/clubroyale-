...
class MockMultiplayerRoom {
  ...
  MockMultiplayerRoom(this.roomId, this.playerIds) : game = MarriageGame() {
  ...
  Map<String, dynamic> playerDeclares(String playerId) {
    ...
    return {'success': false, 'error': 'Invalid hand - cannot declare'};
  }
  ...
  Map<String, dynamic> getRoomState() {
    return {
      ...
      'turnCount': turnCount,
      'hands': {for (final pid in playerIds) pid: game.getHand(pid).length},
    };
  }
}
  ...
  group('Marriage 8-Player Multiplayer Room Simulation', () {
    ...
    test('Large game simulation - 40 turns (5 per player)', () {
      ...
      for (int i = 0; i < turnCount; i++) {
        ...
        room.playerDrawsFromDeck(player);
        expect(
          room.game.getHand(player).length,
          22,
