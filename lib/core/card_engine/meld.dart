...
abstract class Meld {
  ...
  Map<String, dynamic> toJson() {
    return {'type': type.name, 'cardIds': cards.map((c) => c.id).toList()};
  }

  /// Reconstruct a Meld from type and cards
  static Meld fromTypeAndCards(
    MeldType type,
...
class TunnelMeld extends Meld {
  TunnelMeld(List<PlayingCard> cards)
    : super(type: MeldType.tunnel, cards: cards);
...
class DubleeMeld extends Meld {
  DubleeMeld(List<PlayingCard> cards)
    : super(type: MeldType.dublee, cards: cards);
...
class MarriageMeld extends Meld {
  ...
  bool get isValid {
    ...
    final hasJhiplu = cards.any(
      (c) => c.suit == tiplu.suit && c.rank.value == jhipluValue,
    );
    final hasTiplu = cards.any(
      (c) => c.suit == tiplu.suit && c.rank == tiplu.rank,
    );
    final hasPoplu = cards.any(
      (c) => c.suit == tiplu.suit && c.rank.value == popluValue,
    );
...
class ImpureRunMeld extends Meld {
  ...
  bool get isValid {
    ...
    final naturalCards = cards
        .where((c) => !c.isJoker && (helper == null || !helper.isWildcard(c)))
        .toList();
...
class MeldDetector {
  ...
  static List<Meld> findAllMelds(List<PlayingCard> hand, {PlayingCard? tiplu}) {
    ...
    if (tiplu != null) {
      ...
      final hasJhiplu = hand.any(
        (c) => c.suit == tiplu.suit && c.rank.value == jhipluRank,
      );
      final hasTiplu = hand.any(
        (c) => c.suit == tiplu.suit && c.rank == tiplu.rank,
      );
      final hasPoplu = hand.any(
        (c) => c.suit == tiplu.suit && c.rank.value == popluRank,
      );

      if (hasJhiplu && hasTiplu && hasPoplu) {
        final marriageCards = hand
            .where(
              (c) =>
                  c.suit == tiplu.suit &&
                  (c.rank.value == jhipluRank ||
                      c.rank == tiplu.rank ||
                      c.rank.value == popluRank),
            )
            .take(3)
            .toList();
  ...
  static List<ImpureRunMeld> findImpureRuns(
    ...
    for (final entry in bySuit.entries) {
      ...
      for (int start = 0; start < suitCards.length; start++) {
        ...
        for (
          int length = 3;
          length <= 7 && startRank + length - 1 <= 14;
          length++
        ) {
  ...
  static List<ImpureSetMeld> findImpureSets(
    List<PlayingCard> hand,
    PlayingCard tiplu,
  ) {
  ...
  static List<SetMeld> _findSetsWithCard(
    PlayingCard target,
  ...
  static List<RunMeld> _findRunsWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
  ) {
  ...
  static List<TunnelMeld> _findTunnelsWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
  ) {
    final matches = pool
        .where((c) => c.rank == target.rank && c.suit == target.suit)
        .toList();
    if (matches.length < 3) return [];
  ...
  static List<DubleeMeld> _findDubleesWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
  ) {
    final matches = pool
  ...
  static List<MarriageMeld> _findMarriagesWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
    PlayingCard tiplu,
  ) {
    ...
    try {
      final hasJhiplu = pool.firstWhere(
        (c) => c.suit == tiplu.suit && c.rank.value == jhipluRank,
      );
      final hasTiplu = pool.firstWhere(
        (c) => c.suit == tiplu.suit && c.rank.value == tVal,
      );
      final hasPoplu = pool.firstWhere(
        (c) => c.suit == tiplu.suit && c.rank.value == popluRank,
      );
  ...
  static List<ImpureRunMeld> _findImpureRunsWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
    PlayingCard tiplu,
  ) {
      ...
      for (int length = 3; length <= 5; length++) {
        ...
        for (int offset = 0; offset < length; offset++) {
          ...
          if (containsTarget &&
            ...
            final allCards = [
              ...sequenceCards,
              ...wildcards.take(wildcardsNeeded),
            ];
  ...
  static List<ImpureSetMeld> _findImpureSetsWithCard(
    PlayingCard target,
    List<PlayingCard> pool,
    PlayingCard tiplu,
  ) {
    ...
    if (isTargetWild) {
      ...
      final sameRank = naturalCards
          .where((c) => c.rank == target.rank)
          .toList();

      if (sameRank.length >= 2 && wildcards.isNotEmpty) {
        // 2 natural (including target) + 1 wild
        final others = sameRank
