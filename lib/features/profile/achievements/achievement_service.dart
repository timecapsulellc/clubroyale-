...
final userAchievementsProvider = StreamProvider.family<List<Achievement>, String>((ref, userId) {
...
class AchievementService {
  ...
  Future<void> checkGameWinAchievements({
    ...
    if (totalWins >= 1) await updateProgress(userId: userId, achievementId: 'first_win', progress: 1);
    if (totalWins >= 10) await updateProgress(userId: userId, achievementId: 'win_10', progress: totalWins);
    if (totalWins >= 50) await updateProgress(userId: userId, achievementId: 'win_50', progress: totalWins);
    if (totalWins >= 100) await updateProgress(userId: userId, achievementId: 'win_100', progress: totalWins);
    if (totalWins >= 500) await updateProgress(userId: userId, achievementId: 'win_500', progress: totalWins);

    // Game-specific achievements
    switch (gameType) {
      case 'marriage':
        await incrementProgress(userId: userId, achievementId: 'marriage_master');
        break;
      case 'call_break':
        await incrementProgress(userId: userId, achievementId: 'call_break_ace');
        break;
      case 'teen_patti':
        await incrementProgress(userId: userId, achievementId: 'teen_patti_king');
        break;
    }

    // Streak achievements
    if (currentStreak >= 3) await updateProgress(userId: userId, achievementId: 'streak_3', progress: currentStreak);
    if (currentStreak >= 7) await updateProgress(userId: userId, achievementId: 'streak_7', progress: currentStreak);
  }
  ...
  Future<void> checkSocialAchievements({
    ...
    if (friendCount >= 1) await updateProgress(userId: userId, achievementId: 'first_friend', progress: 1);
    if (friendCount >= 10) await updateProgress(userId: userId, achievementId: 'friends_10', progress: friendCount);
    if (friendCount >= 50) await updateProgress(userId: userId, achievementId: 'friends_50', progress: friendCount);

    if (gamesHosted >= 5) await updateProgress(userId: userId, achievementId: 'host_5', progress: gamesHosted);
    if (gamesHosted >= 25) await updateProgress(userId: userId, achievementId: 'host_25', progress: gamesHosted);
  }
  ...
  Future<void> checkDailyStreak({
    ...
    if (consecutiveDays >= 7) {
      await updateProgress(userId: userId, achievementId: 'daily_7', progress: consecutiveDays);
    }
    if (consecutiveDays >= 30) {
      await updateProgress(userId: userId, achievementId: 'daily_30', progress: consecutiveDays);
    }
  }
  ...
  Future<void> grantSpecialAchievement({
    ...
    await updateProgress(userId: userId, achievementId: achievementId, progress: 1);
  }

  /// Get unlocked achievements count
  Future<int> getUnlockedCount(String userId) async {
    final snapshot = await _userAchievementsRef(userId)
  ...
  Future<List<Achievement>> getAllWithProgress(String userId) async {
    ...
    final unlockedDocs = await _userAchievementsRef(userId)
        .where('isUnlocked', isEqualTo: true)
        .get();
