...
class Thought {
  ...
  @override
  String toString() =>
      'Thought($action: $description [${(confidence * 100).toStringAsFixed(0)}%])';
}
...
class ThoughtBranch {
  ...
  factory ThoughtBranch.evaluate({
    ...
    final totalConfidence = thoughts.fold<double>(
      0,
...
class TreeOfThoughtsEngine {
  ...
  List<Thought> _callBreakThoughts(
  ...
  List<Thought> _teenPattiThoughts(
    Map<String, dynamic> context,
    String action,
  ) {
    final thoughts = <Thought>[];
  ...
  List<Thought> _inBetweenThoughts(
    Map<String, dynamic> context,
    String action,
  ) {
    final thoughts = <Thought>[];
  ...
  String _generateReasoning(ThoughtBranch? branch) {
    ...
    final topThought = thoughts.reduce(
