...
final testGameServiceProvider = Provider<TestGameService>((ref) => TestGameService());
...
class TestGameService {
  ...
  Future<void> placeBid(String playerId, int bidAmount) async {
    ...
    _currentGame = _currentGame!.copyWith(
      ...
      currentTurn: allBid ? playerIds.first : CallBreakLogic.getNextPlayer(playerIds, playerId),
    );
  ...
  Future<void> playCard(String playerId, PlayingCard card) async {
    ...
    final hands = Map<String, List<PlayingCard>>.from(_currentGame!.playerHands);
    ...
    if (_currentGame!.currentTrick == null) {
      ...
      currentTrick = _currentGame!.currentTrick!.copyWith(
        cards: [..._currentGame!.currentTrick!.cards, PlayedCard(playerId: playerId, card: card)],
      );
    }
  ...
  Future<void> _autoBotPlay() async {
    ...
    while (_currentGame!.gamePhase == GamePhase.playing &&
           ...
           _currentGame!.currentTurn!.startsWith('bot_')) {
      final botId = _currentGame!.currentTurn!;
      final botHand = List<PlayingCard>.from(_currentGame!.playerHands[botId] ?? []);
      ...
      if (_currentGame!.currentTrick != null) {
        ...
        if (suitCards.isNotEmpty) {
          ...
          final spades = botHand.where((c) => c.suit == CardSuit.spades).toList();
          cardToPlay = spades.isNotEmpty ? spades.first : botHand.first;
        }
      } else {
        // Leading - play any card (prefer non-spade)
        final nonSpades = botHand.where((c) => c.suit != CardSuit.spades).toList();
