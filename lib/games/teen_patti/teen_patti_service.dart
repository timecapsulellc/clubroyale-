  ...
  factory TeenPattiGameState.fromJson(Map<String, dynamic> json) {
    return TeenPattiGameState(
      playerHands:
          ...
          {},
      playerStatus:
          (json['playerStatus'] as Map<String, dynamic>?)?.map(
            (k, v) => MapEntry(k, v as String),
          ) ??
          {},
      playerBets:
          (json['playerBets'] as Map<String, dynamic>?)?.map(
...
class TeenPattiService {
  ...
  String _getNextActivePlayerExcluding(
    ...
    for (int i = 1; i <= players.length; i++) {
      final nextIndex = (currentIndex + i) % players.length;
      final nextPlayerId = players[nextIndex];
      if (nextPlayerId != excludeId &&
          state.playerStatus[nextPlayerId] != 'folded') {
        return nextPlayerId;
      }
    }
