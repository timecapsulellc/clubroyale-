...
enum TeenPattiPhase { dealing, betting, sideShow, showdown, finished }
...
class TeenPattiGame implements BaseGame {
  ...
  // Game tracking
  final int _roundNumber = 0; // Current round counter
  ...
  void initialize(List<String> playerIds) {
    if (playerIds.length < minPlayers || playerIds.length > maxPlayers) {
      throw ArgumentError(
        'Teen Patti requires $minPlayers-$maxPlayers players',
      );
    }
  ...
  bool requestSideShow(String playerId) {
    ...
    int prevIndex =
        (_currentPlayerIndex - 1 + _playerIds.length) % _playerIds.length;
