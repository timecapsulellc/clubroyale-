...
class MarriageBotController {
  ...
  Future<void> _processBotTurn(MarriageGameState state, String botId) async {
    ...
    final updatedHasVisited =
        state.hasVisited(botId) || (await _getUpdatedVisitStatus(botId));

    if (strategy.shouldDeclare(
      hand: hand,
      tiplu: tiplu,
      hasVisited: updatedHasVisited,
    )) {
  ...
  Future<void> _performDraw(
    ...
    // Check discard pile
    final topDiscardId = state.discardPile.isNotEmpty
