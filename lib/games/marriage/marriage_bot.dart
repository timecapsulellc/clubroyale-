...
class MarriageBotStrategy {
  ...
  double get _riskTolerance {
    ...
    switch (difficulty) {
      case MarriageBotDifficulty.easy: return 0.7;
      case MarriageBotDifficulty.medium: return 0.5;
      case MarriageBotDifficulty.hard: return 0.3;
    }
  }
  ...
  double get _aggression {
    ...
    switch (difficulty) {
      case MarriageBotDifficulty.easy: return 0.3;
      case MarriageBotDifficulty.medium: return 0.5;
      case MarriageBotDifficulty.hard: return 0.7;
    }
  }
  ...
  bool shouldDrawFromDeck({
    ...
    required bool canPickFromDiscard, // False if blocked by Joker or visiting rule
  }) {
    ...
    switch (difficulty) {
      ...
      case MarriageBotDifficulty.hard:
        ...
        final potentialMeldCount = _countMeldCards([...hand, topDiscard], tiplu);
        ...
        if (potentialMeldCount > currentMeldCount && _random.nextDouble() < 0.7) {
          return false;
        }
  ...
  Card chooseDiscard({
    required List<Card> hand,
    required Card? tiplu,
    required Card? lastDrawnCard, // Can't discard what we just drew (house rule)
  }) {
    if (hand.isEmpty) {
  ...
  bool shouldDeclare({
    ...
    final isValid = MeldDetector.validateHand(hand, tiplu: hasVisited ? tiplu : null);
  ...
  Card _findBestDiscard(List<Card> candidates, Card? tiplu, {required bool preferHighValue}) {
    ...
    if (loose.isNotEmpty) {
      if (preferHighValue) {
        ...
        loose.sort((a, b) => _connectivity(a, pool).compareTo(_connectivity(b, pool)));
      }
      return loose.first;
    }
  ...
  bool _isWildCard(Card card, Card? tiplu) {
    ...
    if (card.rank == tiplu.rank) {
      final tipluIsRed = tiplu.suit == Suit.hearts || tiplu.suit == Suit.diamonds;
