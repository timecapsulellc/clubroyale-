...
class MarriageGame implements BaseGame {
  ...
  void drawFromDiscard(String playerId) {
    ...
    if (config.mustVisitToPickDiscard && !isVisited(playerId)) {
      throw StateError(
        'Must visit (show pure sets) before picking from discard pile!',
      );
    }
  ...
  bool declare(String playerId) {
    ...
    final effectiveTiplu = isVisited(playerId)
        ? _tiplu
        : null; // Only use wildcards if visited (or check full hand purity)
