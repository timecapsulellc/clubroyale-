...
class MeldAnalysisResult {
  ...
  int get meldedCardCount => melds.fold<int>(0, (sum, m) => sum + m.cards.length);
  ...
  @override
  String toString() => 'MeldAnalysis(melds: ${melds.length}, pure: ${pureSequences.length}, tunnels: ${tunnels.length}, near: ${nearMelds.length}, deadwood: ${deadwood.length})';
}
...
class MarriageMeldAnalyzer {
  ...
  double evaluatePickup(List<Card> hand, Card card) {
    ...
    improvement += (newAnalysis.pureSequences.length - currentAnalysis.pureSequences.length) * 0.3;
    
    // More melds = good
    improvement += (newAnalysis.melds.length - currentAnalysis.melds.length) * 0.15;
    
    // Less deadwood = good  
    improvement += (currentAnalysis.deadwood.length - newAnalysis.deadwood.length) * 0.1;
    
    // More near-melds = some improvement
    improvement += (newAnalysis.nearMelds.length - currentAnalysis.nearMelds.length) * 0.05;
  ...
  List<CardRequirement> getNeededCards(List<Card> hand) {
    ...
    for (final entry in bySuit.entries) {
      ...
      final cards = entry.value..sort((a, b) => a.rank.value.compareTo(b.rank.value));
  ...
  List<NearMeld> _findNearMelds(List<Card> hand, Set<Card> alreadyMelded) {
    ...
    for (final entry in bySuit.entries) {
      ...
      for (int i = 0; i < cards.length - 1; i++) {
        final diff = cards[i + 1].rank.value - cards[i].rank.value;
        if (diff == 1) {
          nearMelds.add(NearMeld(
            cards: [cards[i], cards[i + 1]],
            targetType: MeldType.run,
            neededCards: [
              CardRequirement(rank: CardRank.values.firstWhere(
                (r) => r.value == cards[i].rank.value - 1,
                orElse: () => CardRank.ace,
              ), suit: cards[i].suit),
              CardRequirement(rank: CardRank.values.firstWhere(
  ...
  int _calculateMaal(List<Card> hand) {
    ...
    for (final card in hand) {
      ...
      if (card.suit == tiplu!.suit && (card.rank == CardRank.ace || card.rank == CardRank.king)) {
        maal += 2;
      }
    }
  ...
  Card? _suggestDiscard(List<Card> hand, List<Card> deadwood, Set<Card> meldedCards) {
