...
exports.onVoiceRoomCreated = exports.onFriendshipUpdated = exports.onFriendRequestCreated = exports.onStoryCreated = exports.onSocialMessageSent = void 0;
...
const messaging = () => {
    if (!_messaging)
        _messaging = (0, messaging_1.getMessaging)();
    return _messaging;
};
// =====================================================
// MESSAGE TRIGGERS
 ...
 */
exports.onSocialMessageSent = (0, firestore_1.onDocumentCreated)('chats/{chatId}/messages/{messageId}', async (event) => {
 ...
 */
exports.onStoryCreated = (0, firestore_1.onDocumentCreated)('stories/{storyId}', async (event) => {
 ...
 */
exports.onFriendRequestCreated = (0, firestore_1.onDocumentCreated)('friendships/{friendshipId}', async (event) => {
    ...
    await db().collection('users').doc(initiatedBy).update({
        diamondBalance: firestore_2.FieldValue.increment(5),
        'diamondsByOrigin.socialActivity': firestore_2.FieldValue.increment(5),
    });
});
/**
 ...
 */
exports.onFriendshipUpdated = (0, firestore_1.onDocumentUpdated)('friendships/{friendshipId}', async (event) => {
    ...
    for (const userId of users) {
        await db().collection('users').doc(userId).update({
            diamondBalance: firestore_2.FieldValue.increment(10),
            'diamondsByOrigin.socialActivity': firestore_2.FieldValue.increment(10),
        });
 ...
 */
exports.onVoiceRoomCreated = (0, firestore_1.onDocumentCreated)('voice_rooms/{roomId}', async (event) => {
    const room = event.data?.data();
    const { roomId } = event.params;
    if (!room)
        return;
    const hostId = room.hostId;
    const title = room.title || 'Voice Room';
    // Get host's friends
    const friendsSnapshot = await db()
        .collection('users')
        .doc(hostId)
...
// HELPER FUNCTIONS
// =====================================================
function getMessagePreview(type, text) {
    switch (type) {
        case 'text':
