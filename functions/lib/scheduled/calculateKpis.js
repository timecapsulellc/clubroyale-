...
exports.getKpiDashboard = exports.calculateDailyKpis = void 0;
 ...
 */
exports.calculateDailyKpis = (0, scheduler_1.onSchedule)({
    ...
    const todayStart = firestore_1.Timestamp.fromDate(new Date(today));
    const monthStart = firestore_1.Timestamp.fromDate(thirtyDaysAgo);
    try {
        // DAU - Users active in last 24 hours
        const dauSnap = await db
            .collection("presence")
            .where("lastSeen", ">=", firestore_1.Timestamp.fromDate(oneDayAgo))
            .count()
            .get();
        ...
        const spentSnap = await db
            ...
            .where("type", "==", "spend")
            .get();
        const diamondsSpentToday = spentSnap.docs.reduce((sum, doc) => sum + (doc.data().amount || 0), 0);
        ...
        const earnedSnap = await db
            ...
            .where("type", "==", "earn")
            .get();
        const diamondsEarnedToday = earnedSnap.docs.reduce((sum, doc) => sum + (doc.data().amount || 0), 0);
        ...
        const yesterdayStart = firestore_1.Timestamp.fromDate(oneDayAgo);
 ...
 */
exports.getKpiDashboard = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    if (!request.auth) {
...
//# sourceMappingURL=calculateKpis.js.map