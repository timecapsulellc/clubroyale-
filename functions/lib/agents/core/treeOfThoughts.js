...
exports.treeOfThoughtsFlow = void 0;
const genkit_1 = require("genkit");
const googleai_1 = require("@genkit-ai/googleai");
const logger_1 = require("../../utils/logger");
const metrics_1 = require("../../utils/metrics");
const logger = (0, logger_1.createLogger)('treeOfThoughts');
// Initialize Genkit
 ...
 */
async function generateThoughts(problem, context, count) {
    ...
    `,
        output: {
            schema: genkit_1.z.object({
                thoughts: genkit_1.z.array(genkit_1.z.string()).describe('List of distinct approaches'),
            }),
        },
    });
 ...
 */
async function evaluateThought(thought, problem, context) {
    const { output } = await ai.generate({
        prompt: `
Problem: ${problem}
 ...
 */
async function expandThought(parentThought, problem, context, count) {
    const { output } = await ai.generate({
        prompt: `
Problem: ${problem}
...
Each sub-strategy should be a concrete next step building on the parent approach.
    `,
        output: {
            schema: genkit_1.z.object({
                subThoughts: genkit_1.z.array(genkit_1.z.string()),
            }),
        },
    });
 ...
 */
async function buildThoughtTree(problem, context, config, currentDepth = 0, parentThought = null, iterationCount = { value: 0 }) {
    ...
    for (const thought of thoughts) {
        ...
        if (currentDepth < config.maxDepth - 1 && evaluation.score > 0.6) {
            node.children = await buildThoughtTree(problem, context, config, currentDepth + 1, thought, iterationCount);
        }
        nodes.push(node);
    }
    return nodes;
 ...
 */
function findBestPath(nodes) {
    let bestPath = [];
    let bestScore = 0;
    let bestReasoning = '';
    function traverse(node, currentPath, currentScore) {
 ...
 */
function findAlternativePaths(nodes, count) {
    const allPaths = [];
    function collectPaths(node, currentPath, currentScore) {
        const newPath = [...currentPath, node.thought];
...
// Main ToT Flow
// ============================================
exports.treeOfThoughtsFlow = ai.defineFlow({
    name: 'treeOfThoughts',
    inputSchema: genkit_1.z.object({
        problem: genkit_1.z.string().describe('The problem to solve'),
        context: genkit_1.z.record(genkit_1.z.unknown()).describe('Additional context'),
        config: genkit_1.z.object({
            maxDepth: genkit_1.z.number().optional(),
            branchingFactor: genkit_1.z.number().optional(),
            evaluationThreshold: genkit_1.z.number().optional(),
            maxIterations: genkit_1.z.number().optional(),
        }).optional(),
    }),
    outputSchema: genkit_1.z.object({
        bestPath: genkit_1.z.array(genkit_1.z.string()),
        confidence: genkit_1.z.number(),
        reasoning: genkit_1.z.string(),
        alternativePaths: genkit_1.z.array(genkit_1.z.array(genkit_1.z.string())),
        totalThoughtsGenerated: genkit_1.z.number(),
        executionTimeMs: genkit_1.z.number(),
    }),
}, async (input) => {
    ...
    try {
        // Build thought tree
        const thoughtTree = await buildThoughtTree(input.problem, input.context, config, 0, null, iterationCount);
        ...
        // Track metrics
        await metrics_1.BusinessMetrics.aiAgentAction('treeOfThoughts', 'reason', executionTimeMs);
        ...
        logger.error('Tree of Thoughts failed', error);
        throw error;
    }
});
exports.default = exports.treeOfThoughtsFlow;
