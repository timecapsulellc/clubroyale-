 ...
 */
export const auditGameUpdate = onDocumentUpdated('games/{gameId}', async (event) => {
    ...
    // d) Turn Change (Current Player ID changed)
        ...
        if (action === 'unknown') action = 'endTurn';
    }

    if (action === 'unknown') return; // No significant game move detected
    ...
    if (updateTimeRaw) {
        ...
        if (typeof (updateTimeRaw as any).toMillis === 'function') {
            lastUpdateTime = (updateTimeRaw as any).toMillis();
        } else {
    ...
    await EnhancedAuditService.checkMoveAnomalies({
        roomId: gameId,
        roundNumber: afterState.currentRound || 1,
        playerId: playerId,
        action: action,
