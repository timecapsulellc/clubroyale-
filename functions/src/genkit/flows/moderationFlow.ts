...
const ModerationInputSchema = z.object({
    ...
    roomId: z.string().describe('Game room ID'),
    recentMessages: z.array(z.string()).optional().describe('Recent messages for spam detection'),
});

// Output schema for moderation result
const ModerationOutputSchema = z.object({
    isAllowed: z.boolean().describe('Whether the message should be allowed'),
    reason: z.string().optional().describe('Reason if blocked'),
    category: z.enum([
        'clean',
        'mild_language',
        'profanity',
    ...
    editedMessage: z.string().optional().describe('Censored version of message if mild'),
});
...
- editedMessage: If mild_language, provide censored version (e.g., "f***")`;
        ...
        const { output } = await ai.generate({
            model: geminiFlash, // Use fast model for moderation
            prompt,
